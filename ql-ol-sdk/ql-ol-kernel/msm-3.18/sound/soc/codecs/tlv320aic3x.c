...
struct snd_soc_codec *reload_codec;
/*
...
static struct aic3x_init_reg reload_init_list[] = {
    {2,0xaa},
    {3,0x91},
    {4,0xc0},
    {5,0x00},
...
#define AIC320_RELOAD_INIT_REG_LEN ARRAY_SIZE(reload_init_list)
static int aic320_reg_reload_init(struct snd_soc_codec *codec)
{
	int i;
	u8 reg;
	dev_err(codec->dev, "func: %s, init codec \n", __func__);
	for (i = 0; i < AIC320_RELOAD_INIT_REG_LEN; i++)
	{
		snd_soc_write(codec, reload_init_list[i].reg, reload_init_list[i].val);
		//reg = snd_soc_read(codec, reload_init_list[i].reg);
		//printk(KERN_ERR "********** func:%s, line=%d, reg=0x%x\n", __func__, __LINE__, reg);
	}
	return 0;
}
			   ...
			   struct snd_soc_dai *dai)
{
	...
	printk(KERN_ERR "sundy====>> func:%s, line=%d, reg=0x%x\n", __func__, __LINE__, reg);
	/*
	Grady.quan-2018/09/18
	Refer to [Issue-Depot].[IS0000290][there is no incoming call ring tone in second voice call]
	*/
	if (!aic3x->power)
	{
		printk(KERN_ERR "sundy====>> func:%s, line=%d\n", __func__, __LINE__);
...
static ssize_t codec_reload_init_store(struct device* dev, struct device_attribute *attr,
                        const char* buf, size_t len)
{
        if(buf[0] == '1')
        aic320_reg_reload_init(reload_codec);

        return len;
}


static DEVICE_ATTR(codec_reload_init,0220,NULL,codec_reload_init_store);
			   ...
			   const struct i2c_device_id *id)
{
    ...
    ret = device_create_file(&i2c->dev,&dev_attr_codec_reload_init);
    if (ret) {
