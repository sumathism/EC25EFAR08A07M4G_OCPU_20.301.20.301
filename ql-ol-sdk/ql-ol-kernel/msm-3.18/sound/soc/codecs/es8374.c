...
struct es8374_eq_init_reg {
        u8 reg;
        u16 val;
};

static const struct es8374_eq_init_reg es8374_eqreg_defaults[]= {
{0x08,0x04},
{0x36,0x20},
{0x45,0x01},
{0x46,0x64},
...
#define ES8374_EQ_REG_LEN ARRAY_SIZE(es8374_eqreg_defaults)
static int es8374_eq_reg_init(struct snd_soc_codec *codec)
{
    int i;
    for(i=0;i < ES8374_EQ_REG_LEN;i++) {
        snd_soc_write(codec,es8374_eqreg_defaults[i].reg,es8374_eqreg_defaults[i].val);
        if(i%3==0){
	    msleep(30);		
	}	
    }
...
static int es8374_pcm_shutdown(struct snd_pcm_substream *substream,
                struct snd_soc_dai *dai)
{
        printk("Enter into %s()\n", __func__);
        return 0;
...
//	struct es8374_private *es8374 = snd_soc_codec_get_drvdata(codec);
	...
	case SND_SOC_BIAS_ON:
		...
		snd_soc_write(codec, 0x14, 0x8a);
		snd_soc_write(codec, 0x15, 0x40);
		snd_soc_write(codec, 0x21, 0x20);
		snd_soc_write(codec, 0x22, 0xff);
		snd_soc_write(codec, 0x24, 0x08);
		msleep(30);
		snd_soc_write(codec, 0x1e, 0xa0);
		snd_soc_write(codec, 0x1d, 0x2b);
		snd_soc_write(codec, 0x1c, 0x90);
		snd_soc_write(codec, 0x1a, 0xa0);
		snd_soc_write(codec, 0x09, 0x41);
		snd_soc_write(codec, 0x6d, 0x60);
		snd_soc_write(codec, 0x37, 0x00);
		msleep(30);
		snd_soc_write(codec, 0x36, 0x00);
		snd_soc_write(codec, 0x28, 0x00);
		snd_soc_write(codec, 0x25, 0x00);
		snd_soc_write(codec, 0x38, 0x00);
		break;
	...
	case SND_SOC_BIAS_STANDBY:
		...
		snd_soc_write(codec, 0x28, 0x1c);
		snd_soc_write(codec, ES8374_DAC_CONTROL_REG36, 0x20);
		snd_soc_write(codec, ES8374_DAC_CONTROL_REG37, 0x20);
		msleep(30);
		snd_soc_write(codec, 0x6d, 0x00);
		snd_soc_write(codec, 0x09, 0x80);
		...
		msleep(30);
		snd_soc_write(codec, 0x24, 0x20);
		snd_soc_write(codec, 0x22, 0x00);
		snd_soc_write(codec, 0x21, 0xc0);
		snd_soc_write(codec, 0x15, 0xff);
		snd_soc_write(codec, 0x14, 0x82);
		snd_soc_write(codec, 0x01, 0x03);
		break;
...
static int es8374_probe(struct snd_soc_codec *codec)
{
	...
	snd_soc_write(codec,0x01,0x7F);	//IC clk on
	snd_soc_write(codec,0x05,0x11);	//clk div set
	snd_soc_write(codec,0x0f,0x21);	//bclk invert
	...
	snd_soc_write(codec,0x21,0x60);	//adc set:	SEL LIN1 CH+PGAGAIN=0DB
	snd_soc_write(codec,0x22,0x55);	//adc set:	PGA GAIN=0DB
	snd_soc_write(codec,0x21,0x24);	//adc set:	SEL LIN1 CH+PGAGAIN=0DB
	...
	snd_soc_write(codec,0x1C,0x10);	// spk set
...
static struct of_device_id es8374_if_dt_ids[] = {
	{ .compatible = "quec,quec-max9860-i2c", },
	{ }
};
#endif
...
static ssize_t codec_reg_store(struct device *dev,
                ...
                const char *buf, size_t count)
{
        u8 flag,i2c_reg,i2c_wval,i2c_rval,i;
        struct i2c_client *client = to_i2c_client(dev);
        struct es8374_private *es8374 = i2c_get_clientdata(client);
        struct snd_soc_codec *codec = es8374->codec;
	...
	if (flag==2){             //read2
            //sscanf(buf, "%d %x %x\n",&flag,&i2c_reg,&i2c_wval);
            for (i=0;i<0x6e;i++){
	      i2c_rval=snd_soc_read(codec,i);
	      printk("read,reg=%x,value=%x\n",i,i2c_rval);
            }	
	}
...
#endif
	ret=regmap_read(es8374->regmap,0x0,&val);
    /*    if (val == 0x0) {
                //kfree(es8311);
                return 0;
        }
*/
  ...
  //      quec_set_codec_info("es8374.2-0010", "es8374-hifi");
	...
	*/
	device_create_file(&i2c_client->dev, &dev_attr_codec_reg);
