			     ...
			     struct snd_soc_dai *dai)
{
	...
	ret = regmap_write(max9860->regmap,
			   MAX9860_MICGAIN, 0x28);
	if (ret) {
		dev_err(codec->dev, "Failed to set MICGAIN: %d\n", ret);
		return ret;
	}
			 ...
			 const struct i2c_device_id *id)
{
...
#endif
           //External 26M mclk clock, if there are other requirements later, the at command configuration can be added
		mclk_rate = 26000000;
		max9860->psclk = 1;
    ...
    printk("mclk rate %d pclk rate %d \n",mclk_rate,max9860->pclk_rate);
