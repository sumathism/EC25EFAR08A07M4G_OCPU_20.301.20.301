...
//add by grady, 2017-7-19, add single ended output mode
static u16 init_reg_mono[][2] = {
        {NAU8814_RESET,   		0x0001},
        {NAU8814_POWER1,   		0x015d},
        {NAU8814_POWER2,   		0x0015},
        {NAU8814_POWER3,   		0x00ff},
        {NAU8814_IFACE,    		0x0118},
        {NAU8814_CLOCK,    		0x0000},
        {NAU8814_ADD,      		0x000A},
        {NAU8814_DAC,	 		0x0008},
		...
		{NAU8814_EQ4,			0x0000},
		{NAU8814_EQ5,			0x0000},
        {NAU8814_INPUT,			0x0001},
		{NAU8814_INPPGA,		0x0000},
		{NAU8814_ADCBOOST,		0x0000},
		{NAU8814_SPKMIX,		0x0000},
		{NAU8814_MONOMIX,		0x0001},

		{NAU8814_POWER4,		0x0000},
		{NAU8814_TIMERSLOT,		0x0000},

		{NAU8814_ADCOUTDRIVER,	0x0000},

		{NAU8814_ADD_IF_CTRL,		0x0000},
		{NAU8814_PWR_TIE_OFF_CTRL,	0x0000},
		{NAU8814_AGC_P2P_DET,		0x0000},
		{NAU8814_AGC_PEAK_DET,		0x0000},
		{NAU8814_CTRL_STATUS,		0x0000},
		{NAU8814_OUTPUT_TIE_OFF_CTRL,	0x0130},
};
//end grady

#define SET_CODEC_REG_INIT_NUM  ARRAY_SIZE(init_reg)
#define SET_CODEC_REG_MONO_INIT_NUM  ARRAY_SIZE(init_reg_mono)
	...
	enum snd_soc_bias_level level)
{
	...
	switch (level) {
	  ...
	  case SND_SOC_BIAS_STANDBY:
		  /*
		  Grady.quan-2018/05/03:resolve high current consumption in nau8814 after make a voice
		  Refer to [Issue-Depot].[IS0000137][high current consumption in nau8814 after make a voice]
		  */
		  //power1 |= NAU8814_POWER1_BIASEN | NAU8814_POWER1_BUFIOEN;
		  power1 |= NAU8814_POWER1_BUFIOEN;
...
//add analog output AT command
static int analog_output_value = 0;
static ssize_t nau8814_analog_output_show(struct device *dev, struct device_attribute *attr, char *buf)
{	
	return snprintf(buf, PAGE_SIZE, "%d\n", analog_output_value);
}

static ssize_t nau8814_analog_output_store(struct device *dev, struct device_attribute *attr, const char *buff, size_t size)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct rt8814_priv *rt8814 = i2c_get_clientdata(client);
	struct snd_soc_codec *codec = rt8814->codec;
	...
	sscanf(buff, "%d", &analog_output_value);
	printk("%s, analog_output_value: %d \n",__func__, analog_output_value);
	
	if(analog_output_value == 0)
	{
		for(i = 0; i < SET_CODEC_REG_INIT_NUM; i++)
		{
			nau8814_write(codec,init_reg[i][0],init_reg[i][1]);
		}
	}
	else if(analog_output_value == 1)
	{
		for(i = 0; i < SET_CODEC_REG_MONO_INIT_NUM; i++)
		{
			nau8814_write(codec,init_reg_mono[i][0],init_reg_mono[i][1]);
		}
	}
	else
	{
		pr_err("auxpcm num_slots out of range.\n");
	}
...
static DEVICE_ATTR(nau8814_analog_output, 0644, nau8814_analog_output_show, nau8814_analog_output_store);

static struct device_attribute *aux_pcm_attributes[] = {
	&dev_attr_nau8814_cfg,
	&dev_attr_nau8814_analog_output,
	NULL
};
//end grady
...
static int nau8814_probe(struct snd_soc_codec *codec)
{
	...
	//add by grady, 2017-7-19
	while(attr = *attrs++)
	{
		ret = device_create_file(codec->dev, attr);
		if (ret != 0) {
