...
//add by grady 2017-7-12
static ssize_t auxpcm_num_slots_show
(
	struct device *pdev,
	struct device_attribute *attr,
	char *buf
...
static ssize_t auxpcm_num_slots_store
(
	struct device *pdev,
	struct device_attribute *attr,
	const char *buff, size_t size
)
{
	...
	sscanf(buff, "%d", &num_slots);
	
	if(num_slots == 1)
	{
		auxpcm_pdata->mode_8k.num_slots = num_slots;
		#ifdef CONFIG_QUECTEL_PCM16K_SUPPORT
		auxpcm_pdata->mode_16k.num_slots = num_slots;//16k
		#endif
	}
	else
	{
...
static ssize_t auxpcm_slot_mapping_show
(
	struct device *pdev,
	struct device_attribute *attr,
	char *buf
...
{	
	...
	return snprintf(buf, PAGE_SIZE, "%d\n", auxpcm_pdata->mode_8k.slot_mapping[0]);
}

static ssize_t auxpcm_slot_mapping_store
(
	struct device *pdev,
	struct device_attribute *attr,
	const char *buff, size_t size
)
{
	...
	sscanf(buff, "%d", &slot_mapping);
	
	if(slot_mapping <= 16)
	{
		auxpcm_pdata->mode_8k.slot_mapping[0] = slot_mapping;
		#ifdef CONFIG_QUECTEL_PCM16K_SUPPORT
		auxpcm_pdata->mode_16k.slot_mapping[0] = slot_mapping;//16k
		#endif
	}
	else
	{
		pr_err("auxpcm slot_mapping out of range.\n");
	}
...
)
{
	...
	{		
	    if (test_bit(STATUS_TX_PORT, aux_dai_data->auxpcm_port_status) &&
	        test_bit(STATUS_RX_PORT, aux_dai_data->auxpcm_port_status)) {
		    dev_err(pdev, "%s: PCM ports already set\n", __func__);
		    goto exit;
			}
	}
#endif
...
static DEVICE_ATTR(num_slots, S_IRUGO | S_IWUSR, auxpcm_num_slots_show, auxpcm_num_slots_store);
static DEVICE_ATTR(slot_mapping, S_IRUGO | S_IWUSR, auxpcm_slot_mapping_show, auxpcm_slot_mapping_store);
