...
/* Add codec indicate status from pcm-daemon 20190924 gale */
static bool quec_codec_status = false;
static DEFINE_MUTEX(quec_codec_status_mutex);
...
static int mdm_sec_auxpcm_startup(struct snd_pcm_substream *substream)
{
	...
	if (atomic_inc_return(&sec_aux_ref_count) == 1) {
...
#ifdef CONFIG_QUECTEL_AUDIO_DRIVER
		if (strstr(quec_codec_name, "max9860-codec")) {
			fmt = 0;
			if (pdata->sec_auxpcm_mode == 1) {
		...
		if (!strcmp(quec_codec_name, "max9867.2-0019")) {
			fmt = 0;
            if (pdata->sec_mi2s_mode == 1) {
            fmt = SND_SOC_DAIFMT_CBS_CFS
        ...
        ret = snd_soc_dai_set_fmt(codec_dai, fmt);
            ...
            pdata->mclk_freq = 26000000;
            ret = snd_soc_dai_set_sysclk(codec_dai, 0, pdata->mclk_freq, 0);
            printk("********** %s   :  %d   mclk = %d\n",__func__,__LINE__,pdata->mclk_freq);
            if (ret < 0)
            {
                printk("********** %s   :  %d\n",__func__,__LINE__);
                return ret;
...

{
        ...
        pr_info("%s dev driver name:%s\n", __func__,
         ...
         if (!strcmp(codec_dev_driver_name, "es8311")) {
                if (pdata->sec_auxpcm_mode == 1) {
                ...
                ret = snd_soc_dai_set_fmt(codec_dai, fmt);//fmt master
                if (ret < 0)
                        return ret;
        }

        if (!strcmp(codec_dev_driver_name, "es8374")) {
                if (pdata->sec_auxpcm_mode == 1) {
                        fmt = SND_SOC_DAIFMT_CBS_CFS|SND_SOC_DAIFMT_IB_NF|SND_SOC_DAIFMT_DSP_A;
                } else {
                        fmt = SND_SOC_DAIFMT_CBM_CFM|SND_SOC_DAIFMT_IB_NF|SND_SOC_DAIFMT_DSP_A;
	...
	if (!strcmp(codec_dev_driver_name, "rt5616e-codec")) {
        	if (pdata->sec_auxpcm_mode == 1) {
       		     fmt = SND_SOC_DAIFMT_CBS_CFS
...
{
	mutex_lock(&quec_codec_status_mutex);
	
	memcpy(quec_codec_name, buff, size);
	quec_codec_name[size-1] = '\0';

	quec_codec_status = true;
	mutex_unlock(&quec_codec_status_mutex);
...
static int ql_app_set_pcm_mode = 0;//app set pcm mode after,audio probe not need read pcm mode in device tree
static int ql_pcm_mode = 1;
...
)
{
	...
	if(mode == I2S_PCM_SLAVE_MODE || mode == I2S_PCM_MASTER_MODE)
	{
		...
		ql_app_set_pcm_mode = 1;
		ql_pcm_mode = (u16)mode;
	}
	else
	{
		pr_err("mode error");	
					...
					struct snd_soc_card *card)
{
	...
	if (size < 0) {
        ...
        //app set pcm mode after,audio probe not need read pcm mode in device tree
		if(ql_app_set_pcm_mode == 1)
		{
		    printk("use config from pcm_mode_select,pcm_Mode:%d\n",ql_pcm_mode);
			pdata->sec_auxpcm_mode = ql_pcm_mode;
			return ret;
		}
...
static int mdm_asoc_machine_probe(struct platform_device *pdev)
{
	...
	*/
	...
	/* Judge codec from pcm-daemon 20190924 gale */
	mutex_lock(&quec_codec_status_mutex);
	if(quec_codec_status == false)
	{
		pr_info("[%s]: wait indicate codec from pcm-daemon\n", __func__);
		ret = -EPROBE_DEFER;
		mutex_unlock(&quec_codec_status_mutex);
		goto err;
	}
	...
	if (ret == -EPROBE_DEFER) {
		...
		panic("snd_soc_register_card failed: alloc memory error");
	} else if (ret) {
...
static int mdm_asoc_machine_remove(struct platform_device *pdev)
{
	...
	mutex_destroy(&quec_codec_status_mutex);
