...
#endif
//add by robin OPEN version default enter dump from ST sai
...
static int __init quectel_set_subsystem_aprstlevel(char *str)
{
	...
	pr_info("[Aurel] system_set_show_reset_mode %d\n", system_reset_mode);

	return 0;
}

early_param("aprstlevel", quectel_set_subsystem_aprstlevel);
...
static int modem_reset_mode = 1; //0 - dload, 1 - reset
/* add by [jared.lu] 20180515 for add modemrstlevel default value*/
...
static int __init quectel_set_modemrstlevel(char *str)
{
	...
	pr_info("[Aurel] modemrstlevel str %s\n", str);

	kstrtouint(str, 0, &rstlevel);

	modem_reset_mode = rstlevel;

	pr_info("[Aurel] modemrstlevel %d\n", modem_reset_mode);

	return 0;
}

static int __init quectel_set_dumpmode(char *str)
{
        if (str) {
		system_reset_mode=0;
		modem_reset_mode=0;
        ...
        pr_info("[gary] modemrstlevel %d, aprstlevel %d\n", modem_reset_mode, system_reset_mode);
        return 0;
}

early_param("modemrstlevel", quectel_set_modemrstlevel);
early_param("dump", quectel_set_dumpmode);
