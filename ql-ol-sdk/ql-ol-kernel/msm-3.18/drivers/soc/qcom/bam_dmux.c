...
// quectel modify
#define SHUTDOWN_TIMEOUT_MS	8000
#define UL_WAKEUP_TIMEOUT_MS 10000
...
//add by tinker.sun, 2020/05/28,  fix can not shutdown
#ifdef QUECTEL_POWER_DOWN
    if (quec_system_power_downed()){
		mutex_unlock(&wakeup_lock);
        DBG("%s() quectel_powerdown\n", __func__);
        return;
    }
#endif
	...
	if (wait_for_ack) {
		while (1) {
			...
			if (ret == 0) {
				// timeout
				DMUX_LOG_KERR("%s:%d, UL_WAKEUP timeout, force restart", __func__, __LINE__);
				emergency_restart();
			}
			else 
			{
