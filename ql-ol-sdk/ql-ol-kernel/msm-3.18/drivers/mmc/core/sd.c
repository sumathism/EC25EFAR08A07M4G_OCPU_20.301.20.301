...
static int mmc_sd_init_uhs_card(struct mmc_card *card)
{
	...
	} else if(!(card->host->caps & MMC_CAP_4_BIT_DATA)) { /*20220212 zoffy,add, sd-card 1-bit mode and 4-bit mode switch*/
	...
	struct mmc_card *oldcard)
{
	...
	if (rocr & SD_ROCR_S18A) {
		...
		} else if(!(host->caps & MMC_CAP_4_BIT_DATA)) {/*20220212 zoffy,add, sd-card 1-bit mode and 4-bit mode switch*/
			err = mmc_app_set_bus_width(card, MMC_BUS_WIDTH_1);
			if (err)
				goto free_card;
