...
/*[Quinn]GNSS requires 2.85v power supply.set sd_vdd to 2.85v when poweroff off.*/
#define POWEROFF_SET_SD_VDD_HIGH 1
...
#define S_LOG_D(fmt, ...) printk("[QL-SDIO-D] %d %s() " fmt "\n", __LINE__, __func__, ##__VA_ARGS__)
	const char *sdio_clk_rates_chosen_match = NULL;
	u32 sdio_clk_rates_chosen = 0;
	const char *const sdio_clk_rates_node_match[] = {
		"qcom,clk-rates-0",
		"qcom,clk-rates-1",
		"qcom,clk-rates-2",
		"qcom,clk-rates-3",
...
/*_Harry 20230328 AT+QCFG="SDIO_CLK" command to set the sdio clk. Change the throughput of wifi.*/
#ifdef QUECTEL_SDIO_CLK_RATE_REGULATE
	if (!of_property_read_string(np, "qcom,clk-rates-chosen",
								 &sdio_clk_rates_chosen_match))
	{
		S_LOG_D("clock rates chosen is presented, going to sdio clk selecting loading procedure");
		
		sdio_clk_rates_chosen = *sdio_clk_rates_chosen_match - '0';
		S_LOG_D("clk table chosen:%c, %d, %x, %s", 
				*sdio_clk_rates_chosen_match,
				sdio_clk_rates_chosen,
				sdio_clk_rates_chosen & 0x7,
				sdio_clk_rates_node_match[sdio_clk_rates_chosen & 0x7]);
		if (sdhci_msm_dt_get_array(dev,
								   sdio_clk_rates_node_match[sdio_clk_rates_chosen & 0x7],
								   &clk_table,
								   &clk_table_len,
								   0))
		...
		S_LOG_D("clk table len:%d", clk_table_len);
	}
	else
	{
		S_LOG_D("clock rates chosen didn't present, going to legacy loading procedure\n");
...
static irqreturn_t sdhci_msm_pwr_irq(int irq, void *data)
{
	...
	if (irq_status & CORE_PWRCTL_BUS_OFF) {
		...
		if (!ret) {
		   ...
		   if(msm_host->is_sdhc2) {
		/*[Quinn]GNSS requires 2.85v power supply.set sd_vdd to 2.85v when poweroff off.*/
                #if POWEROFF_SET_SD_VDD_HIGH
			ret |= sdhci_msm_set_vdd_io_vol(msm_host->pdata,
					VDD_IO_HIGH, 0);
                #else
			ret |= sdhci_msm_set_vdd_io_vol(msm_host->pdata,
					VDD_IO_LOW, 0);
					#endif //POWEROFF_SET_SD_VDD_HIGH
			}
			else {
				ret |= sdhci_msm_set_vdd_io_vol(msm_host->pdata,
						VDD_IO_LOW, 0);
...
/*20220212 zoffy,add, sd-card 1-bit mode and 4-bit mode switch*/
static ssize_t
bus_mode_switch_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct sdhci_host *host = dev_get_drvdata(dev);
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	struct sdhci_msm_host *msm_host = pltfm_host->priv;
	int value = 0;
	value = msm_host->mmc->caps & MMC_CAP_4_BIT_DATA;
	if(value == 1)
		value = 4;
	else if(value == 0)
		value = 1;
	return snprintf(buf, PAGE_SIZE, "%d-bit mode.\n", value);
}

static ssize_t
bus_mode_switch_store(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t count)
{
	struct sdhci_host *host = dev_get_drvdata(dev);
	struct sdhci_pltfm_host *pltfm_host = sdhci_priv(host);
	...
	if (!kstrtou32(buf, 0, &value)) {
		if(value == 4) {
			...
			msm_host->mmc->caps &= (~0x00000001);
			msm_host->pdata->mmc_bus_width == 0;
		}
	}
	return count;
}
...
static int sdhci_msm_probe(struct platform_device *pdev)
{
	...
	/*Quinn add sdhc2 flag*/
#if 1
        if (!strcmp(dev_name(&pdev->dev), "7864900.sdhci")) {
            msm_host->is_sdhc2= true;
        }
#endif
    ...
    if(msm_host->is_sdhc2) {
	/*Quinn add to reduces SD Card speed to high speed*/
...
//	msm_host->mmc->caps2 |= MMC_CAP2_CLK_SCALE; //add by cullen ,20190926 some sd card can not be recongne
	...
	/*if (!gpio_is_valid(msm_host->pdata->status_gpio)) {*/ /*Quinn comment for SD Card Detect*/
	...
	/*}*/ /*Quinn comment for SD Card Detect*/
	...
	/*20220212 zoffy,add, sd-card 1-bit mode and 4-bit mode switch*/
	msm_host->bus_mode.show = bus_mode_switch_show;
	msm_host->bus_mode.store = bus_mode_switch_store;
	sysfs_attr_init(&msm_host->bus_mode.attr);
	msm_host->bus_mode.attr.name = "bus_mode";
	msm_host->bus_mode.attr.mode = S_IRUGO | S_IWUSR;
	ret = device_create_file(&pdev->dev, &msm_host->bus_mode);
	if (ret) {
		device_remove_file(&pdev->dev, &msm_host->bus_mode);
	}
