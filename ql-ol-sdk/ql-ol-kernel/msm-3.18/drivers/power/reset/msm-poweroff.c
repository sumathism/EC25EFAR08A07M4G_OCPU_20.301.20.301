...
//<2019/02/01-QCM9X00229-P01-Vicent.Gao, <[CALL] Segment 1==> Add new feature: BOOT_ENTER_DUMP.>
//#define QUECTEL_BOOT_ENTER_DUMP 
//>2019/02/01-QCM9X00229-P01-Vicent.Gao
...
//<2019/02/01-QCM9X00229-P01-Vicent.Gao, <[CALL] Segment 1==> Add new feature: BOOT_ENTER_DUMP.>
#if defined(QUECTEL_BOOT_ENTER_DUMP)
static int download_mode = 1;
#else //#if defined(QUECTEL_BOOT_ENTER_DUMP)
static int download_mode = 0;
#endif // #if defined(QUECTEL_BOOT_ENTER_DUMP)
//>2019/02/01-QCM9X00229-P01-Vicent.Gao
#else
static int download_mode = 1;
#endif
...
static int __init quectel_set_msm_power_aprstlevel(char *str)
{
	unsigned int rstlevel;

	pr_info("[Jared] aprstlevel str %s\n", str);

	kstrtouint(str, 0, &rstlevel);

	download_mode = ((rstlevel == 0) ? 1 : 0);

	pr_info("[Aurel] download_mode %d\n", download_mode);

	return 0;
}

early_param("aprstlevel", quectel_set_msm_power_aprstlevel);
...
extern void ql_set_nand_layer_write_forbidden(void);
...
static void do_msm_poweroff(void)
{
	...
	// add by hans @20201204 "add write_forbidden attribute file to forbidden MTD write" start
	ql_set_nand_layer_write_forbidden();
	// add by hans @20201204 "add write_forbidden attribute file to forbidden MTD write" end
	// add by hans @20201126 "it will delay 15ms before fast shutdown" start
	mdelay(15);
	// add by hans @20201126 "it will delay 15ms before fast shutdown" end
...
// add by hans @20201126 "with pin fast shutdown" start
void ql_set_fastshutdown_with_pin(void)
{
	do_msm_poweroff();
}
EXPORT_SYMBOL(ql_set_fastshutdown_with_pin);
...
/*zoffy_20220106_added the node(/sys/kernel/dload/reset_shutdown_timer) to force shutdown or reboot after the set time.*/
enum timer_type{
	RESET_TYPE = 1,
	SHUTDOWN_TYPE = 2,
};
#define RESET_SHUTDOWN_TIMER_LENGTH_S 15
struct timer_list shutdown_or_reboot_timer_s;

static void force_reset_shutdown_handler(unsigned long cmd)
{
	switch(cmd) {
		case RESET_TYPE:
			do_msm_restart(REBOOT_SOFT, NULL);
			break;
		case SHUTDOWN_TYPE:
			do_msm_poweroff();
			break;
...
static size_t store_timer_to_force_power_off(struct kobject *kobj, struct attribute *attr,
				const char *buf, size_t count)
{
	struct timer_list *shutdown_or_reboot_timer = &shutdown_or_reboot_timer_s;
	uint32_t timer_type = 0;
	int ret;
	static int exec_count = 0;
	ret = kstrtouint(buf, 0, &timer_type);
	if (ret < 0)
		return ret;
	if (!timer_type)
		return -EINVAL;
	...
	init_timer(shutdown_or_reboot_timer);
	shutdown_or_reboot_timer->function = &force_reset_shutdown_handler;
	shutdown_or_reboot_timer->expires = jiffies + RESET_SHUTDOWN_TIMER_LENGTH_S*HZ;
	if(timer_type == SHUTDOWN_TYPE)
		shutdown_or_reboot_timer->data = SHUTDOWN_TYPE;
	else if(timer_type == RESET_TYPE)
		shutdown_or_reboot_timer->data = RESET_TYPE;
	add_timer(shutdown_or_reboot_timer);
	exec_count ++;
	return count;
}
RESET_ATTR(reset_shutdown_timer, 0644, NULL, store_timer_to_force_power_off);
//end zoffy_20220106
#endif
//end jun.wu
...
/*zoffy_20220106_added the node(/sys/kernel/dload/reset_shutdown_timer) to force shutdown or reboot after the set time.*/
	&reset_attr_reset_shutdown_timer.attr,
#endif
//end jun.wu
...
static int msm_restart_probe(struct platform_device *pdev)
{
	...
	//modify by dawn [2018-10-15] fix up not find dload dir.
