...
static int emac_probe(struct platform_device *pdev)
{
	...
	/*zoffy.yu add,20230327,Special treatment for some phy probabilistic register reading failure.*/
	hw->quirk_phy_flag = 1;
	...
	msleep(20);
	ret = adpt->gpio_off(adpt, true, true);
	if (ret)
		goto err_clk_init;
	...
	/*zoffy.yu add,20230327,Special treatment for some phy probabilistic register reading failure.*/
	if(adpt->phydev->phy_id == BCM89832_PHY_ID)
		hw->quirk_phy_flag = 1;
	else 
		hw->quirk_phy_flag = 0;
...
err_clk_init:
	//will.shao, fix bug STMDM9X07-6523
	if (adpt->phydev && ((ATH8030_PHY_ID == adpt->phydev->phy_id) ||
	    (ATH8031_PHY_ID == adpt->phydev->phy_id) ||
	    (ATH8035_PHY_ID == adpt->phydev->phy_id) ||
	    (BCM89820_PHY_ID == adpt->phydev->phy_id) ||
	    (BCM89832_PHY_ID == adpt->phydev->phy_id) ||
