...
//gunner.yang fix sleep failed when modem restar
/* Remove the DIAG_WS wakeup source, becasue ap can`t suspend when modem reset */
...
static unsigned int diag_flag;
static unsigned int diag_flag_bit;
...
int ql_rawdata_read_usb_diag_flag(void)
{	
	ql_system_cfg_type *usbcfg_1;
	struct rawdata_item item_usb1;

	if(quec_rawdata_read_usb_cfg(&item_usb1) == 0){
		usbcfg_1 = (ql_system_cfg_type*)&item_usb1.value.data[0];
		diag_flag = usbcfg_1->usb_diag_flag;
		printk("[QUEC][DM]: ql_rawdata_read_usb_diag_flag:%d\n\n",diag_flag);
	}
	return 0;
}
