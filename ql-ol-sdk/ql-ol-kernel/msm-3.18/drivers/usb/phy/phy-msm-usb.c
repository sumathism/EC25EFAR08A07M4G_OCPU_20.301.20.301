...
void quec_force_udc_suspend_for_vbus_aways_online(void);
#endif
...
static void force_usb_suspend_work_cb(struct work_struct *w)
{
	...
	}else if (motg->vbus_state) {
		/*hanwen add,20220422,for MINI_PCIE,Vbus and VBAT are connected,the usb can not enter low power mode when the USB is not plugged and device is powered on.*/
		pr_debug("[phy-msm-usb] motg->vbus_state=%d.\n", motg->vbus_state);
		quec_force_udc_suspend_for_vbus_aways_online();
	}
}
#endif
...
#ifdef QUECTEL_SLEEP_CTRL
	msm_otg_dbg_log_event(phy, "LPM ENTER DISABLE motg, phy IRQ",
			usb_port_suspend(motg), phy->state);
    if (phy->state == OTG_STATE_B_SUSPEND && !usb_port_suspend(motg))
    {
	msm_otg_dbg_log_event(phy, "LPM ENTER DP is resume",
			motg->inputs, phy->state);
        ret = -EBUSY;
        goto phy_suspend_fail;
    }
...
phcd_retry:
	...
	msm_otg_dbg_log_event(phy, "LPM ENTER ENABLE IRQ",
			motg->inputs, phy->state);
...
#ifdef QUECTEL_SLEEP_CTRL
static int ql_usb_resume_bus_vote(struct msm_otg *motg)
{
	//if (motg->device_bus_suspend && debug_bus_voting_enabled)
	//Wythe.WANG-20200511
	//remove the condition of bus suspend. In some case, OTG state
	//turn to b_suspend, but the device bus isn't suspend, it should
	//not vote min perf
...
static int msm_otg_resume(struct msm_otg *motg)
{
	 ...
	 */
#ifdef QUECTEL_SLEEP_CTRL
	if (motg->phy.state != OTG_STATE_B_SUSPEND)
	{
		if (motg->device_bus_suspend && debug_bus_voting_enabled)
			msm_otg_bus_vote(motg, USB_MAX_PERF_VOTE);
		else
...
static void msm_otg_sm_work(struct work_struct *w)
{
	...
	bool work = 0, dcp;
	int ret;
#if QUECTEL_FAKE_VBUS
	int	i_cnt, fake_vbus_down;	// added by ndh for test

	pr_debug("%s work\n", usb_otg_state_string(otg->phy->state));
	pr_err("[NDH] %s: otg_state : %s => ", __func__, usb_otg_state_string(otg->phy->state));
#endif
	...
	case OTG_STATE_B_PERIPHERAL:
		if (test_bit(B_SESS_VLD, &motg->inputs) &&
			...
			if(!fake_vbus_down) {
				...
				pr_err("[NDH] %s: workaround for fake VBUS down #1.\n", __func__);
			work = 1;
#endif
	...
	case OTG_STATE_B_SUSPEND:
		if (!test_bit(B_SESS_VLD, &motg->inputs)) {
			...
			work = 1;
				...
				pr_err("[NDH] %s: workaround for fake VBUS down #2.\n", __func__);
			work = 1;
#endif
		} else if (!test_bit(A_BUS_SUSPEND, &motg->inputs)) {
			pr_debug("!a_bus_suspend\n");
