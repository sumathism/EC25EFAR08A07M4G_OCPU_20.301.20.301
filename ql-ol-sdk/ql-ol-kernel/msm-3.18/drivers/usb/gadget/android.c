...
struct e_state_statistics
{
	unsigned char  disconnect_state_count;
	unsigned char  connected_state_count;
	unsigned char  configured_state_count;
	unsigned char  suspend_state_count;
	unsigned char  resumed_state_count;
} usb_enum_state;
...
static void android_work(struct work_struct *data)
{
	...
	if (uevent_envp) {
		...
		if(uevent_envp == disconnected) {
			usb_enum_state.disconnect_state_count ++;
		} else if (uevent_envp == connected) {
			usb_enum_state.connected_state_count ++;
		} else if (uevent_envp == configured) {
			usb_enum_state.configured_state_count ++;
		} else if (uevent_envp == suspended) {
			usb_enum_state.suspend_state_count ++;
		} else if (uevent_envp == resumed) {
			usb_enum_state.resumed_state_count ++;
		}
...
static ssize_t mbim_product_info_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%s\n", mbim_product_info);
}

static ssize_t mbim_product_info_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	char *end = NULL;
	
	strlcpy(mbim_product_info, buf, sizeof(mbim_product_info));
	end = mbim_product_info + strlen(mbim_product_info) -1;
	if('\n' == *end) {
		*end = '\0';
	}
	return size;
...
static DEVICE_ATTR(mbim_product_info, S_IRUGO | S_IWUSR,
				mbim_product_info_show, mbim_product_info_store);

static struct device_attribute *mbim_function_attributes[] = {
	&dev_attr_mbim_transports,
	&dev_attr_wMTU,
	&dev_attr_mbim_product_info,
	NULL
};
...
// add by barry 2019/12/26 for uac music

//static int uac_fs;
static char uac_music_play[40]  = "/dev/snd/pcmC0D16p"; 
static char uac_music_cap[40]  = "/dev/snd/pcmC0D16c";

static int audio_enable = 0;
static int fs;
int uac_fs_set(struct usb_function *func, int fs);
int quec_audio_enable(struct usb_function *func, int enable);

//int quec_audio_uac_fs(struct usb_function *func, int fs);
int quec_audio_music_play(struct usb_function *func, char* play);
int quec_audio_music_cap(struct usb_function *func, char* cap);
...
static ssize_t uac_music_play_show(struct device *dev,
                struct device_attribute *attr, char *buf)
{
       	return snprintf(buf, PAGE_SIZE, "%s\n", uac_music_play);
	
}
static ssize_t uac_music_play_store
(
	struct device *dev,
...
)
{
	struct android_usb_function *f = dev_get_drvdata(dev);
        struct audio_function_config *config = f->config;
	memcpy(uac_music_play, buff, size);
	uac_music_play[size-1] = '\0';
	printk("uac play:%s\n",uac_music_play);
	quec_audio_music_play(config->func,uac_music_play);
...
static ssize_t uac_music_cap_show(struct device *dev,
                struct device_attribute *attr, char *buf)
{
        return snprintf(buf, PAGE_SIZE, "%s\n",uac_music_cap);
        
}
static ssize_t uac_music_cap_store
(
        struct device *dev,
        struct device_attribute *attr,
        const char *buff, size_t size
)
{
        struct android_usb_function *f = dev_get_drvdata(dev);
        struct audio_function_config *config = f->config;
        memcpy(uac_music_cap, buff, size);
        uac_music_cap[size-1] = '\0';
        printk("uac cap:%s\n",uac_music_cap);
        quec_audio_music_cap(config->func,uac_music_cap);

        return size;
}
static ssize_t uac_fs_show(struct device *dev,
                struct device_attribute *attr, char *buf)
{
        return snprintf(buf, PAGE_SIZE, "%d\n", fs);

}
static ssize_t uac_fs_store
(
        struct device *dev,
        struct device_attribute *attr,
        const char *buff, size_t size
...
static DEVICE_ATTR(uac_fs, S_IRUGO | S_IWUSR, uac_fs_show,
                                uac_fs_store);

static DEVICE_ATTR(play_pcm, S_IRUGO | S_IWUSR, uac_music_play_show,
                                uac_music_play_store);
static DEVICE_ATTR(cap_pcm, S_IRUGO | S_IWUSR, uac_music_cap_show,
                                uac_music_cap_store);
...
static char SerialPortType[10]="NA";	/*enabled FSERIAL ports - "tty[,sdio]"*/
...
struct QuecUsbMarkOpenState QuecSerialPortState;
					...
					struct usb_configuration *c)
{
	...
	struct serial_function_config *config = f->config;
	char QuecSerialbuf[10],*QuecSerial;
	...
	strlcpy(QuecSerialbuf, SerialPortType, sizeof(QuecSerialbuf));
	QuecSerial = strim(QuecSerialbuf);

	if(usb_cfg.usb_fixed_interface == 1)
	{
		memset(&QuecSerialPortState, 0,sizeof(QuecSerialPortState));
	}
	...
	while (b) {
		...
		if(usb_cfg.usb_fixed_interface == 1)
		{
			...
			{
				QuecSerialPortState.NmeaMarkOpenState=1;
			}
			else if ((strcmp(name,"smd_at")== 0)&&(QuecSerialPortState.SMDATMarkOpenState==0))
			{
				QuecSerialPortState.SMDATMarkOpenState=1;
			}
			else if ((strcmp(name,"smd_modem") == 0)&&(QuecSerialPortState.SMDModemMarkOpenState==0))
			{
				QuecSerialPortState.SMDModemMarkOpenState=1;
			}
		}
		if (name) {
			if (tb)
...
int usb_dtr_enable = 0;
EXPORT_SYMBOL(usb_dtr_enable);
static ssize_t dtr_enable_show(struct device *pdev, struct device_attribute *attr,
			   char *buf)
{
        return snprintf(buf, PAGE_SIZE, "%d\n", usb_dtr_enable);
}

static ssize_t enum_state_statistics_show(struct device *pdev, struct device_attribute *attr,
			   char *buf)
{
        return snprintf(buf, PAGE_SIZE, "\"USB_STATE=DISCONNECTED\" times:%d\n\"USB_STATE=CONNECTED\" times:%d\n\"USB_STATE=CONFIGURED\" times:%d\n\"USB_STATE=SUSPENDED\" times:%d\n\"USB_STATE=RESUMED\" times:%d\n", 
                usb_enum_state.disconnect_state_count, usb_enum_state.connected_state_count, usb_enum_state.configured_state_count, usb_enum_state.suspend_state_count, usb_enum_state.resumed_state_count);
}

static ssize_t dtr_enable_store(struct device *pdev, struct device_attribute *attr,
			    const char *buff, size_t size)
{
        sscanf(buff, "%d", &usb_dtr_enable);
        return size;
}
...
static DEVICE_ATTR(dtr_enable, S_IRUGO | S_IWUSR, dtr_enable_show, dtr_enable_store);
static DEVICE_ATTR(enum_state_statistics, S_IRUGO, enum_state_statistics_show, NULL);
...
static struct device_attribute *android_usb_attributes[] = {
	...
	&dev_attr_enable,
	&dev_attr_dtr_enable,
	&dev_attr_enum_state_statistics,
	&dev_attr_pm_qos,
...
int ql_rawdata_read_usb_cfg(void)
{	
	ql_system_cfg_type *usbcfg;
	struct rawdata_item item_usb1;

	memset(&usbcfg, 0, sizeof(usbcfg));
	memset(&usb_cfg, 0, sizeof(usb_cfg));
	
	if(quec_rawdata_read_usb_cfg(&item_usb1) == 0){
		usbcfg = (ql_system_cfg_type*)&item_usb1.value.data[0];
		usb_cfg.usbnet = usbcfg->usbnet;
		usb_cfg.usb_fixed_interface = usbcfg->usb_fixed_interface;
		pr_info("ql_usb_cfg usb_fixed_interface: %d,usbnet: %d\n", usb_cfg.usb_fixed_interface,usb_cfg.usbnet);
	}
	return 0;
}
