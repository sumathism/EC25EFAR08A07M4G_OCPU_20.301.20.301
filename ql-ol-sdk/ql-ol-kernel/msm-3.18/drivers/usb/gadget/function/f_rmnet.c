...
extern unsigned int ql_remove_tx_zlp;
#endif
...
#ifdef QUECTEL_REMOVE_TX_ZLP
static void
frmnet_set_remove_tx_zlp_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	memcpy(&cfg, req->buf, sizeof(cfg));
	ql_remove_tx_zlp = le32_to_cpu(cfg.enable);
}
#endif
...
frmnet_setup(struct usb_function *f, const struct usb_ctrlrequest *ctrl)
{
	...
	case ((USB_DIR_OUT | USB_TYPE_CLASS | USB_RECIP_INTERFACE) << 8)
		...
		ret = w_length;
		req->complete = frmnet_set_remove_tx_zlp_complete;
		req->context = dev;
		break;
#endif
