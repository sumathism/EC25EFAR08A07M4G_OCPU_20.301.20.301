...
//Wythe.WANG-20201105 add to avoid data sent 
//when host system didn't open port
#define QUECTEL_USB_DTR_CONTROL

#ifdef QUECTEL_USB_DTR_CONTROL
static int sDTR = 0;
extern int usb_dtr_enable;
#endif
		...
		struct usb_request *req)
{
	...
	/*Since windows diag driver couldn't support to send DTR event,
	in order to avoid diag port blocked on windows system, we'll set
	DTR to high when diag port receive message from host system*/
#ifdef QUECTEL_USB_DTR_CONTROL
        if (1 == usb_dtr_enable) {
...
#ifdef QUECTEL_USB_DTR_CONTROL
	//Wythe.WANG-20201127
	/*Restore sDTR to 0 when diag port disconnect*/
	sDTR = 0;
#endif
	spin_lock_irqsave(&ch_lock, flags);
	ch->priv = NULL;
...
#ifdef QUECTEL_USB_DTR_CONTROL
	if(usb_dtr_enable == 1 && !sDTR)
	{
		pr_debug("Skip diag data\n");
		return -EAGAIN;
	}
#endif
...
#ifdef QUECTEL_USB_DTR_CONTROL
#define DIAG_CTRL_RTS	(1 << 1)
#define DIAG_CTRL_DTR	(1 << 0)
static int
diag_ser_setup(struct usb_function *f, const struct usb_ctrlrequest *ctrl)
{
        if (usb_dtr_enable == 0)
                return -EOPNOTSUPP;

	struct diag_context *gdiag = func_to_diag(f);
	...
	case ((USB_DIR_OUT | USB_TYPE_CLASS | USB_RECIP_INTERFACE) << 8)
		...
		pr_debug("%s: USB_CDC_REQ_SET_CONTROL_LINE_STATE: DTR:%d RST:%d\n",
			__func__, w_value & DIAG_CTRL_DTR ? 1 : 0,
			w_value & DIAG_CTRL_RTS ? 1 : 0);
		sDTR = w_value & DIAG_CTRL_DTR ? 1 : 0;
		break;
...
#ifdef QUECTEL_USB_DTR_CONTROL
	dev->function.setup = diag_ser_setup;
#endif
	kref_init(&dev->kref);
