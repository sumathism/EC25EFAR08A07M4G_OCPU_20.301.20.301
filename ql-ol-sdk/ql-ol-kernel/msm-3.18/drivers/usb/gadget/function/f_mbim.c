...
/* string descriptors: */
static char mbim_product_info[33];
...
static char mbim_function_name[][32] = {
	"usb_mbim_ctrl",
	"usb_mbim_data",
	"usb_mbim"
};
enum mbim_interface_name_m {
	MBIM_CTRL,
	MBIM_DATA,
	MBIM,
};
...
mbim_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	{
		strcpy(f->name,(const char*)mbim_function_name[MBIM_CTRL]);
	}
	status = usb_interface_id(c, f);
	if(usb_cfg.usb_fixed_interface == 1)
	{
	strcpy(f->name,(const char*)mbim_function_name[MBIM]);
	}
	if (status < 0)
		goto fail;
	mbim->ctrl_id = status;
	...
	if(usb_cfg.usb_fixed_interface == 1)
	{
	strcpy(f->name,(const char*)mbim_function_name[MBIM_DATA]);
	f->intf_id = -1;
	}
					 ...
					 char *xport_name)
{
	...
	mbim_string_defs[STRING_CTRL_IDX].s = mbim_product_info;
