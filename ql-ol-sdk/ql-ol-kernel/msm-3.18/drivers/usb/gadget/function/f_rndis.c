...
#if 1 //add by carl.yin, clear the cache rndis msg, or rndis_response_available donot notify PC
		if (atomic_read(&rndis->notify_count)) {
			u8 *buf;
			u32 n;

			buf = rndis_get_next_response(rndis->config, &n);
			while (buf) {
				print_hex_dump(KERN_CONT, "rndis_set_alt", DUMP_PREFIX_OFFSET, 32, 1, buf, n, false);
				rndis_free_response(rndis->config, buf);
				buf = rndis_get_next_response(rndis->config, &n);
			}
			atomic_set(&rndis->notify_count, 0);
...
static char rndis_function_name[][32] = {
	"rndis_ctrl",
	"rndis_data",
	"rndis"
};
enum rndis_interface_name_m {
	RNDIS_CTRL,
	RNDIS_DATA,
	RNDIS,
};
...
rndis_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	{
	strcpy(f->name,(const char*)rndis_function_name[RNDIS_CTRL]);
	}
	status = usb_interface_id(c, f);
	if(usb_cfg.usb_fixed_interface == 1)
	{
	strcpy(f->name,(const char*)rndis_function_name[RNDIS]);
	}
	if (status < 0)
		goto fail;
	...
	if(usb_cfg.usb_fixed_interface == 1)
	{
	strcpy(f->name,(const char*)rndis_function_name[RNDIS_DATA]);
	f->intf_id = -1;
	}
	status = usb_interface_id(c, f);
	if(usb_cfg.usb_fixed_interface == 1)
