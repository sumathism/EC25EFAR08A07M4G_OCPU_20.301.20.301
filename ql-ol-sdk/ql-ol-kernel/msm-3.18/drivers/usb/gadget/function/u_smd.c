...
//Wythe.WANG-20201127 add to avoid reboot URC sent from USBAT
...
#ifdef QUECTEL_USB_DTR_CONTROL
		if(false == port->is_dtr_high && usb_dtr_enable == 1)
		{
			pr_debug("%s: skip data at dtr status %d\n", __func__, port->is_dtr_high);
                        // add by allen.yang 2020.1.15 for Discard the data uploaded by the modem
...
#ifdef QUECTEL_USB_DTR_CONTROL
	if (usb_dtr_enable == 1)
		port->is_dtr_high = ctrl_bits & SMD_ACM_CTRL_DTR ? true : false;
#endif
...
void gsmd_disconnect(struct gserial *gser, u8 portno)
{
	...
	port->is_suspended = false;
	//Wythe.WANG-20201127
	//Restore sDTR to 0 when smd disconnect
#ifdef QUECTEL_USB_DTR_CONTROL
	if (usb_dtr_enable == 1)
		port->is_dtr_high = false;
#endif
	spin_unlock_irqrestore(&port->port_lock, flags);
