...
//Wythe.WANG-20201105, add to avoid serial state sent 
//when host system didn't open the port
		...
		void *data, unsigned length)
{
	...
	if(usb_dtr_enable == 1)
	{
		pr_debug("Skip serial state notify\n");
		return -EAGAIN;
	}
#endif
...
static struct usb_function *gser_alloc(struct usb_function_instance *fi)
{
	...
	{
		if((QuecSerialPortState.NmeaMarkOpenState==1)&&((QuecSerialPortState.InitMark & 0x02)!=0x02/*nmea port flag*/))
		{
			gser->port.func.name = "nmea";
			QuecSerialPortState.InitMark |=0x02;
		}
		else if((QuecSerialPortState.SMDATMarkOpenState==1)&&((QuecSerialPortState.InitMark & 0x01)!=0x01/*at port flag*/))
		{
			gser->port.func.name = "smd_at";
			QuecSerialPortState.InitMark |=0x01;
		}	
		else if((QuecSerialPortState.SMDModemMarkOpenState==1)&&((QuecSerialPortState.InitMark & 0x04)!=0x04/*modem port flag*/))
		{
			gser->port.func.name = "smd_modem";
			QuecSerialPortState.InitMark |=0x04;
		}
	}
