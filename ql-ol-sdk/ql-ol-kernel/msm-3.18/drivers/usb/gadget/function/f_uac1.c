...
static int uac_fs_config_read(void)
{
    struct file *fp;
    mm_segment_t fs;
    loff_t pos;
    ...
    int uac_fs;
    int ret;
    printk("enter into uac_fs_config_read");
    fp = filp_open("/etc/uac_fs.conf",O_RDWR|O_CREAT, 0644);
    if (IS_ERR(fp)) {
    	    printk("uac_fs.conf error\n");
           return 0;
    }
...
static char audio_function_name_g[][32] = {
	"g_audio_ctrl",
	"g_audio_microphone",
	"g_audio_speaker",
	"g_audio"
};
enum ecm_interface_name_m {
	AUDIO_CTRL,
	AUDIO_MIRCOPHONE,
	AUDIO_SPEAKER,
	AUDIO_,
...
f_audio_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	{
	strcpy(f->name,(const char *)audio_function_name_g[AUDIO_CTRL]);
	}
	status = usb_interface_id(c, f);
	if(usb_cfg.usb_fixed_interface == 1)
	{
	strcpy(f->name,(const char *)audio_function_name_g[AUDIO_]);
	}
	if (status < 0) {
	...
	if(usb_cfg.usb_fixed_interface == 1)
	{
	strcpy(f->name,(const char *)audio_function_name_g[AUDIO_MIRCOPHONE]);
	f->intf_id = -1;
	}
	...
	if(usb_cfg.usb_fixed_interface == 1)
	{
	strcpy(f->name,(const char *)audio_function_name_g[AUDIO_SPEAKER]);
	f->intf_id = -1;
	}
	status = usb_interface_id(c, f);
	if(usb_cfg.usb_fixed_interface == 1)
...
{
    struct f_uac1_opts * opt;
    opt = container_of(func->fi, struct f_uac1_opts, func_inst);
    opt->sample_rate = fs;
...
int quec_audio_music_play(struct usb_function *func, char* play)
{
    ...
    opt = container_of(func->fi, struct f_uac1_opts, func_inst);
    opt->fn_play = play;
    if(!strcmp(play,"/dev/snd/pcmC0D16p"))
	{
 	   opt->audio_playback_buf_size = 320;
	   uac_music_set = 1;
	}
	return 0;
}
int quec_audio_music_cap(struct usb_function *func, char* cap)
{
   	 ...
   	 opt = container_of(func->fi, struct f_uac1_opts, func_inst);
   	 opt->fn_cap = cap;
	if(!strcmp(cap,"/dev/snd/pcmC0D0c"))
   	     opt->audio_capture_buf_size = 320;
	return 0;
}
int quec_uac_open_snd_dev(struct gaudio *card, int open);
static struct usb_function *quec;
...
void quec_uac_music_enbale(int enable)
{
    struct f_audio		*audio;
    audio = func_to_audio(quec);
    quec_uac_open_snd_dev(&audio->card, enable);
}
EXPORT_SYMBOL(quec_uac_music_enbale);
#endif
