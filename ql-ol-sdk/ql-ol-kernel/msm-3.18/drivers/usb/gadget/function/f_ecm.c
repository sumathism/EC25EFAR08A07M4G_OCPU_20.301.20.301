...
static char ecm_function_name[][32] = {
	"cdc_ethernet_ctrl",
	"cdc_ethernet_data",
	"cdc_ethernet"
};
...
ecm_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	{
		strcpy(f->name,(const char *)ecm_function_name[ECM_CTRL]);
	}
	...
	if(usb_cfg.usb_fixed_interface == 1)
	{
		strcpy(f->name,(const char *)ecm_function_name[ECM]);
	}
	...
	if(usb_cfg.usb_fixed_interface == 1)
	{
		strcpy(f->name,(const char *)ecm_function_name[ECM_DATA]);
		f->intf_id = -1;
	}
