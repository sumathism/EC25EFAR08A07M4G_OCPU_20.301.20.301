...
retry:
	for (p = fs_names; *p; p += strlen(p)+1) {
		...
		if (!get_bootmode(NULL))
		{
			...
			Quectel_Set_Partition_RestoreFlag("system",-1,1);
            // modify by [francis.huan] 20180417 ,for match mtd name to restore
		}
        else
        {
			printk("@Ramos set restore systemfs flag here 111 \r\n");
			Quectel_Set_Partition_RestoreFlag("recoveryfs",-1,1);
            // modify by [francis.huan] 20180417 ,for match mtd name to restore
		}
#endif
	...
	__bdevname(ROOT_DEV, b);
		...
		if (!get_bootmode(NULL))
		{
			...
			Quectel_Set_Partition_RestoreFlag("system",-1,2);
            // modify by [francis.huan] 20180417 ,for match mtd name to restore
		}else{
			Quectel_Set_Partition_RestoreFlag("recoveryfs",-1,2);
            // modify by [francis.huan] 20180417 ,for match mtd name to restore
		}
#endif
