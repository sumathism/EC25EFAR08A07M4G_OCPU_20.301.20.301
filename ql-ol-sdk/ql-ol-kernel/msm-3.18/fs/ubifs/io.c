...
void ubifs_ro_mode(struct ubifs_info *c, int err)
{
	if (!c->ro_error) {
		...
		dump_stack();
		// add by hans @20201126 "mount error restore point" start
		printk("@hans UBI Error  10101111 ubifs_check_node vi.dev_num=%d, vi.vol_id=%d , \r\n",  (c)->vi.ubi_num,(c)->vi.vol_id);
		Quectel_Set_Partition_RestoreFlag("",ubi_get_device(c->vi.ubi_num)->mtd->index,10);
		// add by hans @20201126 "mount error restore point" end
	}
}
...
******************************************************************************************/
	printk("@Ramos UBI Error 6663333  ubifs_read_node_wbuf vi.dev_num=%d, vi.vol_id=%d , \r\n",  (c)->vi.ubi_num,(c)->vi.vol_id);
...
<由于ubi文件系统损坏，低概率的出现读写分区变成只读分区导致部分功能无法使用 变砖（增加还原点规避）>
******************************************************************************************/
	printk("@Ramos UBI Error 6664444  ubifs_read_node_wbuf vi.dev_num=%d, vi.vol_id=%d , \r\n",  (c)->vi.ubi_num,(c)->vi.vol_id);
#if 1 // def  QUECTEL_MODEM_BACKUP    //Ramos 20160801 add for modem file backup
	//Quectel_Set_Partition_RestoreFlag("",ubi_get_device(c->vi.ubi_num)->mtd->index,6);
#endif	
		dump_stack();
