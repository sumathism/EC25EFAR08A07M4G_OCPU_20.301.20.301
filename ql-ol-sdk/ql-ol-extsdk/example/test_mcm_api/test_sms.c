                            ...
                            void*               contextPtr)
{
	...
	else if(msgRef->type == E_QL_SMS_TYPE_BROADCAST_RX)
	{
...
static int test_sms(void)
{
    ...
    while(1)
    {
        ...
        case 1://"QL_SMS_Send_Sms"
        {
            ...
            quectel_set_recvsms_format(e_format); //config recvice sms format

            if((e_format == E_QL_SMS_FORMAT_GSM_7BIT) || (e_format == E_QL_SMS_FORMAT_IRA) || (e_format == E_QL_SMS_FORMAT_UCS2))
            {
            ...
            if(e_format == E_QL_SMS_FORMAT_GSM_7BIT || e_format == E_QL_SMS_FORMAT_IRA)
            {
        ...
        case 4://"QL_SMS_SetRouteList"
        {
            ...
            t_route_list.route_list[E_QL_SMS_MESSAGE_CLASS_0].route_storage  = E_QL_SMS_STORAGE_TYPE_NONE;  
        ...
        case 7:
        {
            ...
            ret = QL_SMS_Get_Routes_List(h_sms, &t_route_list);
            printf("QL_SMS_Get_Routes_List ret=%d \n", ret);

            for(i = 0; i < E_QL_SMS_MESSAGE_CLASS_MAX; i++)
            {
                printf("class%d: route_memory=%d  route_value=%d\n", t_route_list.route_list[i].message_class,
                    t_route_list.route_list[i].route_storage, t_route_list.route_list[i].receipt_action);
            }
        ...
        case 8:
        {
            ...
            char *sms_format[] = {"GSM-7", "Binary", "UCS2", "IRA"};
            ...
            t_sms_storage_info.e_mode = E_QL_SMS_MESSAGE_MODE_GW;
            ret = QL_SMS_Read_Msg(h_sms, &t_sms_storage_info, &t_sms_info);
            printf("QL_SMS_Read_Msg ret=%d \n", ret);

            if(ret != 0)
                break;

            if(t_sms_info.user_data_head_valid)
            {
                printf("\n%s msg from %s at %s, total_segments:%d, seg_number:%d, len:%d, content: ", sms_format[t_sms_info.format], t_sms_info.src_addr,
                    t_sms_info.timestamp, t_sms_info.user_data_head.total_segments, t_sms_info.user_data_head.seg_number, t_sms_info.sms_data_len);
            }
            else
            {
                printf("\n%s msg from %s at %s, len:%d, content: ", sms_format[t_sms_info.format], t_sms_info.src_addr,
                    t_sms_info.timestamp, t_sms_info.sms_data_len);
            }
            
            if(t_sms_info.format == E_QL_SMS_FORMAT_GSM_7BIT)
            {
                printf("%s\n", t_sms_info.sms_data);
            }
            else if(t_sms_info.format == E_QL_SMS_FORMAT_BINARY_DATA)
            {
                int i;
                for(i = 0; i < t_sms_info.sms_data_len; i++)
                {
                    printf("%.2X ", t_sms_info.sms_data[i]);
                }
                printf("\n");
            }
            else if(t_sms_info.format == E_QL_SMS_FORMAT_UCS2)
            {
                unsigned char* smsbuf = NULL;

                smsbuf = (char*)malloc(sizeof(char)*QL_SMS_MAX_MT_MSG_LENGTH);
                UnicodeStrToUTF8Str((unsigned short*)(&t_sms_info.sms_data[0]), smsbuf, QL_SMS_MAX_MT_MSG_LENGTH);
                printf("%s\n", smsbuf);
            }
            break;
        }
