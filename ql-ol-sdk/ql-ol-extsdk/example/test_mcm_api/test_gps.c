                                 ...
                                 void                    *contextPtr)
{
    ...
    switch(e_msg_id)
    {
        ...
        {
            int i;
            QL_LOC_GNSS_SV_STATUS_T  *pt_svinfo = (QL_LOC_GNSS_SV_STATUS_T *)pv_data;

            for(i=0; i<pt_svinfo->num_svs; i++)
            {
			printf("num_svs[%d][%d]: svid=%d, constellation=%d, c_no_dbhz=%f, elevation=%f, azimuth=%f\n",
                    pt_svinfo->num_svs,
					i, 
                    pt_svinfo->sv_list[i].svid,
                    pt_svinfo->sv_list[i].constellation,
                    pt_svinfo->sv_list[i].c_n0_dbhz,
