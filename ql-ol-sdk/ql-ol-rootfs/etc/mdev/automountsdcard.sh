#! /bin/sh

#add by [asa.wang] 20180325 from 9x07/R05 start
#if [ -d /media/sdcard ]
#then
#	echo "media/card exit!"
#else
#	mkdir -p /media/sdcard
#fi

#destdir=/media/sdcard
#add by [asa.wang] 20180325 from 9x07/R05 end
if [ -e /tmp/media_flag ];then
    if [ ! -d /data/ufs/media ];then
        mkdir -p /data/ufs/media
    fi
    destdir=/data/ufs/media
else
    if [ -d /media/sdcard ]
    then
        echo "media/card exit!"
    else
        mkdir -p /media/sdcard
    fi
    destdir=/media/sdcard
fi

umount_partition()
{
        if grep -qs "^/dev/$1 " /proc/mounts ; then
                umount -lf "${destdir}";
		#asa.wang-2018/03/25: modify sdcard mount point
		rm -rf "${destdir}";
        fi
}

mount_partition()
{
        if [ ! -d "${destdir}" ]; then
            mkdir "${destdir}"
        fi
        if ! mount -t auto "/dev/$1" "${destdir}"; then
                # failed to mount
                exit 1
        fi
}

check_if_boot_dev()
{
                ret_val=`cat /proc/cmdline | grep "androidboot.bootdevice" |wc -l`
                if  [ $ret_val -eq 1 ]; then
                    boot_dev=`cat /proc/cmdline | awk '{ for ( n=1; n<=NF; n++ ) if($n ~ "androidboot.bootdevice") print $n }' | awk '{split($0,a, "=");print a[2]}'`
                    real_sysfs_path=`realpath /sys/class/block/$1`
                    ret_val=`echo $real_sysfs_path | grep /sys/devices/ | grep $boot_dev | wc -l`
                fi
                echo $ret_val
}


create_symlink()
{
                real_sysfs_path=`realpath /sys/class/block/$1`
                partition_name=`cat $real_sysfs_path/uevent | awk '{ for ( n=1; n<=NF; n++ ) if($n ~ "PARTNAME") print $n }' | awk '{split($0,a, "=");print a[2]}'`
                mkdir -p /dev/block/bootdevice/by-name/
                partition_name=/dev/block/bootdevice/by-name/$partition_name
                target_dev=/dev/$1
                ln -s $target_dev $partition_name
}

case "${ACTION}" in
add|"")
        result=`check_if_boot_dev $1`
        if [ $result -eq 1 ]; then
                 create_symlink $1 &
        else
                 umount_partition ${1}
                 mount_partition ${1}
        fi
        ;;
remove)
        umount_partition ${1}
        ;;
esac

