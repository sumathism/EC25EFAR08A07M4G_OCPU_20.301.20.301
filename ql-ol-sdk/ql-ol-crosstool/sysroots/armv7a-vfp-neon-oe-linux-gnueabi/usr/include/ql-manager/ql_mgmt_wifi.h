...
/*============================================================================
  ...
  10/11/2018       Mike        Adds WiFi ssid support hexdump.
  12/28/2018       Mike        Adds WiFi station scan event.
  01/30/2019       Mike        WiFi station show signal.
============================================================================*/
...
typedef enum {
	...
	QL_MGMT_CLIENT_COMMAND_WIFI_STA_SCAN                           = 0x0018,
	QL_MGMT_CLIENT_COMMAND_WIFI_STA_SCAN_RESULT                    = 0x0019,
	QL_MGMT_CLIENT_COMMAND_WIFI_ACCESS_PROFILE                     = 0x001a
...
struct ql_mgmt_client_wifi_essid {
	wifi_index_e_v01 id;
	wifi_ssid_format_type_e_v01 fmt_type;
	char ssid[33];
};
...
struct ql_mgmt_client_wifi_mac_acls {
	wifi_index_e_v01 id;
	unsigned int cnt;
	unsigned int rule; /*keep the rule of MAC filter*/
...
struct access_point {
	...
	int channel;
	wifi_auth_e_v01 auth;
	int flags; //Add the wps algorithm flag bit by harry.gong
};
...
struct ql_mgmt_client_wifi_station_scan {
	wifi_index_e_v01 id;
};

struct ql_mgmt_client_wifi_station_scan_event {
	wifi_index_e_v01 id;
};

struct ql_mgmt_client_wifi_station_scan_result {
	wifi_index_e_v01 id;
	int cnt;
	struct access_point access_point[WIFI_MAX_STA_SCAN_RESULT_CNT_V01];
};

struct ql_mgmt_client_wifi_access_profile {
	bool state;
};
...
typedef struct {
	union {
		...
		struct ql_mgmt_client_wifi_station_scan sta_scan;
		struct ql_mgmt_client_wifi_station_scan_result sta_scan_result;
		struct ql_mgmt_client_wifi_access_profile access_profile;
	};
