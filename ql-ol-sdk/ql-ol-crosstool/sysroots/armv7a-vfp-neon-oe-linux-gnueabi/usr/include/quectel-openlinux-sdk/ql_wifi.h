...
typedef enum {
  WIFI_AP_PROFILE_FULL_ACCESS = 0,       /**<  AP Can Access complete LAN Including Internet  */
  WIFI_AP_PROFILE_INTERNET_ONLY = 1,     /**<  AP Can Access Only Internet  */
}ql_wifi_ap_access_profile_e;
...
typedef enum {
	...
	QL_WIFI_STATION_AUTH_FAIL       /* WiFi station authentication failed */
...
typedef enum {
	...
	QL_WIFI_AUTH_WPS_ONLY              /* WPS ONLY */
} ql_wifi_auth_e;
...
/**
 * Set the WiFi configuration: the hotspot country code.
 *
 * @param [in]   the hotspot country code,the size must be 2 bytes.
 *
 * @return 
 *   On success, 0 is returned.  On error, -1 is returned.
 * 
 */
extern int ql_wifi_country_code_set(char *country_code);

/**
 * Get the WiFi configuration: the hotspot country code.
 *
 * @param [in]    the hotspot country code
 * @param [in]    the size of hotspot country code
 *
 * @return 
 *   On success, 0 is returned.  On error, -1 is returned.
 * 
 */
extern int ql_wifi_country_code_get(char *country_code,int country_code_size);

/**
 * Scan on the given frequencies and probe for the given SSIDs unless passive scanning is requested.
 * After the interface is called, it will return the QL_WIFI_EVENT_STA_SCAN_DONE event in about 10 seconds. 
 * At this time, you can use the ql_wifi_sta_scan_result interface to get the interface for scanning the hotspots around.
 *
 * @param None
 *
 * @return 
 ...
 * 
 */
extern int ql_wifi_sta_scan_set(void);

/**
 * Dump the current scan results.
 *
 * @param [out]   scan results
 *
 * @return 
 *   On success, 0 is returned.  On error, -1 is returned.
 * 
...
extern int ql_wifi_sta_scan_result(ql_wifi_sta_scan_result_s *result);
...
/*switch WIFI module between FC20/FC21, they use different golden bin
*/
int ql_wifi_module_switch(ql_wifi_module_e wmodule); 

/**
 * Set the WiFi configuration: ap_isolate of hotspot configuration.
 *
 * @param [in]    Index of AP mode
 * @param [in]    If set ENABLE, Enable isolation
 *                If set DISABLE, Disable isolation
 *
 * @return 
 *   On success, 0 is returned.  On error, -1 is returned.
 * 
 */
extern int ql_wifi_ap_isolate_set(ql_wifi_ap_index_e idx, bool isolate);

/**
 * Get the WiFi configuration: ap_isolate of hotspot configuration.
 *
 * @param [in]    Index of AP mode
 * @param [in]    Get the hotspot isolation status
 *
 * @return 
 *   On success, 0 is returned.  On error, -1 is returned.
 * 
 */
extern int ql_wifi_ap_isolate_get(ql_wifi_ap_index_e idx, bool *isolate);

/**
 * Set the WiFi configuration: Sets guest ap access profile.
 *
 * @param [in]    Index of AP mode
 * @param [in]    If set true, AP Can Access Only Internet.
 *                If set false, AP Can Access complete LAN Including Internet.
 *
 * @return 
 *   On success, 0 is returned.  On error, -1 is returned.
 * 
 */
extern int ql_wifi_access_profile_set(ql_wifi_ap_access_profile_e access_profile);

/**
 * Get the WiFi configuration: Gets guest ap access profile.
 *
 * @param [in]    Index of AP mode
 * @param [in]    Get the guest ap access profile.
 *
 * @return 
 *   On success, 0 is returned.  On error, -1 is returned.
 * 
 */
extern int ql_wifi_access_profile_get(ql_wifi_ap_access_profile_e *access_profile);
