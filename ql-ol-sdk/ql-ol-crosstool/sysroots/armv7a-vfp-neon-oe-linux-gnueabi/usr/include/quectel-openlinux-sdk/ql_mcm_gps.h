...
typedef struct 
{
    ...
    float azimuth;
}QL_LOC_GNSS_SV_INFO_T;                      /* Type */

typedef struct 
{
    uint32_t            size;                                   /**<   Set to the size of mcm_gps_sv_status_t. */
    int                 num_svs;                                /**<   Number of SVs currently visible. */
    QL_LOC_GNSS_SV_INFO_T    sv_list[QL_LOC_GPS_SUPPORT_SVS_MAX];    /**<   Contains an array of SV information. */
}QL_LOC_GNSS_SV_STATUS_T;  /* Type */
...
int QL_LOC_GET_PREDICTED_ORBITS_DATA_VALIDITY(loc_client_handle_type h_loc, 
                                        QL_LOC_XTRA_DATA_VALIDITY *validityt_info);
