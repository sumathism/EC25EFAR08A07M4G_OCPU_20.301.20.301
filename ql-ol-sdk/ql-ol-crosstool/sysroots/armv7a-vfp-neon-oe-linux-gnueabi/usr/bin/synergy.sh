...
bt_cod_default="0x000400,0x000008"

show_usage()
{
    echo "Usage: ./synergy {start|stop|restart|on|off} {carkit|phone}"
}
...
#verify bt_cod, by Victor
if [ -n "${bt_cod}" ]
then
	echo "${bt_cod}"
else
	echo "${bt_cod_default}"
	bt_cod=${bt_cod_default}
fi
...
SYNERGY_APP=quectel_ble_service
...
# UART port
#UART_PORT=/dev/ttyHS0 #rock change to ttyHS1 for toyota 
UART_PORT=/dev/ttyHS1
...
#    stop_bt_audio_service
   ...
   if [ -S /tmp/.ble_socket_server ]
   then
	echo "Info==Going to remove /tmp/.ble_socket_server"
	rm /tmp/.ble_socket_server
   fi
}
...
start_synergy_app_for_carkit()
{
    ...
    if [ "$enable_log" = "0" ]; then
        $SYNERGY_APP_FULL_NAME --bc-transport $TRANSPORT --bc-port $PORT --bc-baud $INIT_BAUDRATE --bc-bs-baud $NEW_BAUDRATE --platform $PLATFORM --bc-bs-device-name $MTP_NAME --bt-dev carkit --bt-cod ${bt_cod} --bt-remote-address 2424:0E:CA7E44 --ibs $IBS --lpm $LPM  &
    else
        $SYNERGY_APP_FULL_NAME --bc-transport $TRANSPORT --bc-port $PORT --bc-baud $INIT_BAUDRATE --bc-bs-baud $NEW_BAUDRATE --platform $PLATFORM --bc-bs-device-name $MTP_NAME --bt-dev carkit --bt-cod ${bt_cod} --bt-remote-address 2424:0E:CA7E44 --ibs $IBS --lpm $LPM --log-cleartext-stdout --log-pcap-file $WIRESHARE_LOG --log-btsnoop-file $HCI_LOG --tui-nocls &
    fi
}
...
{
    echo "phone"
	#the following if-branch is added by Victor
	if [ -f ${BT_SCDB_PATH}  ]
	then
		#echo "${BT_SCDB_PATH} is deleted!" > /data/bt_999.log
		rm ${BT_SCDB_PATH}
	fi	

    if [ "$enable_log" = "0" ]; then
        $SYNERGY_APP_FULL_NAME --bc-transport $TRANSPORT --bc-port $PORT --bc-baud $INIT_BAUDRATE --bc-bs-baud $NEW_BAUDRATE --platform $PLATFORM --bc-bs-device-name $MTP_NAME --bt-dev phone --bt-av-wav-file-in /data/csr.wav --bt-remote-address 0002:5B:00FF07 --ibs $IBS --lpm $LPM &
    else
        $SYNERGY_APP_FULL_NAME --bc-transport $TRANSPORT --bc-port $PORT --bc-baud $INIT_BAUDRATE --bc-bs-baud $NEW_BAUDRATE --platform $PLATFORM --bc-bs-device-name $MTP_NAME --bt-dev phone --bt-av-wav-file-in /data/csr.wav --bt-remote-address 0002:5B:00FF07 --ibs $IBS --lpm $LPM --log-cleartext-stdout --log-pcap-file $WIRESHARE_LOG --log-btsnoop-file $HCI_LOG --tui-nocls &
    fi
}
